...
docker compose -f "$ROOT_DIR/docker-compose.yml" exec -T redis sh -lc 'redis-cli del freelist >/dev/null 2>&1 || true; redis-cli sadd freelist "{\"ip\":\"10.0.0.2\"}" >/dev/null'
...
echo "[test] Borrowing item should succeed immediately..."
