...
docker compose -f "$ROOT_DIR/docker-compose.yml" exec -T redis sh -lc 'redis-cli del ip_freelist >/dev/null 2>&1 || true; redis-cli sadd ip_freelist 10.0.0.3 >/dev/null'
...
echo "[test] Borrowing IP should succeed..."
